...
class FDMPrinter(printer3D.Printer3D):
    ...
    def __init__(self):
        ...
        self.addSetting('platform_adhesion', Setting('raft_line_spacing', 5.0, 'float').setRange(0).setLabel(_("Raft line spacing (mm)"), _("Distance between the raft lines.\nThe first 2 layers of the raft have this amount of spacing between the raft lines.")))
        self.addSetting('platform_adhesion', Setting('raft_base_thickness', 0.3, 'float').setRange(0).setLabel(_("Raft base thickness (mm)"), _("Layer thickness of the first raft layer. This should be a thick layer which attaches firmly to the printer bed.")))
        self.addSetting('platform_adhesion', Setting('raft_base_linewidth', 1.0, 'float').setRange(0).setLabel(_("Raft base line width (mm)"), _("Width of the first raft layer lines.\nThese should be thick lines to assist in bed adhesion.")))
        self.addSetting('platform_adhesion', Setting('raft_base_speed', 15.0, 'float').setRange(0).setLabel(_("Raft base print speed (mm/s)"), _("Speed at which the first raft layer is printed. This should be printed quite slow, as the amount of material coming out of the nozzle is quite high.")))
        self.addSetting('platform_adhesion', Setting('raft_interface_thickness', 0.27, 'float').setRange(0).setLabel(_("Raft interface thickness (mm)"), _("Thickness of the 2nd raft layer.")))
        self.addSetting('platform_adhesion', Setting('raft_interface_linewidth', 0.4, 'float').setRange(0).setLabel(_("Raft interface line width (mm)"), _("Width of the 2nd raft layer lines. These lines should be thinner then the first layer but strong enough to attach the object to.")))
        self.addSetting('platform_adhesion', Setting('raft_airgap', 0.22, 'float').setRange(0).setLabel(_("Raft air-gap (mm)"), _("Gap between the final raft layer and the first layer of the object. Only the first layer is raised by this amount to lower the bonding between the raft layer and the object. Makes it easier to peel off the raft.")))
        self.addSetting('platform_adhesion', Setting('raft_surface_layers', 2, 'float').setRange(0).setLabel(_("Raft surface layers"), _("Amount of surface layers on top of the 2nd raft layer. These are fully filled layers used to print the object on top of. 2 layers usually works fine.")))
    ...
    def getNozzleOffset(self, nozzle_nr):
        if nozzle_nr < 1:
            x, y = 0.0, 0.0
        else:
            x = self.getSettingValueByKeyFloat('machine_nozzle_offset_x_%d' % (nozzle_nr))
            y = self.getSettingValueByKeyFloat('machine_nozzle_offset_y_%d' % (nozzle_nr))
